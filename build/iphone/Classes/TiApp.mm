...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kNursingBlogAppv2UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kNursingBlogAppv2UserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
